...
public class Floor2
{
    ...
    public Floor2(FloorType floorType, int number, Vector3 elementScale)
    {
    ...
    private void Create(int number, Vector3 elementScale, FloorType floorType)
    {
        int length = _floorType.PlacesCount;
        _places = new Place[length];
        Vector2 rectangleSize = new Vector2(elementScale.x * floorType.Basis.x + elementScale.z, elementScale.z * floorType.Basis.y + elementScale.x);
        Rectangle rectangle = new Rectangle(Vector2.zero, rectangleSize);
